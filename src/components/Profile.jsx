...
export const Profile=(props)=>{
    ...
    for( var i=0;i<props.blogs.length;i++){
        if(props.blogs[i].id==id){
            props.setBlogs([...props.blogs.splice(0,i), ...props.blogs.splice(i+1)]);
        }
    }
    }
                ...
                return (<>
                    ...
                    <div class="c3">
                      <Btn2>
                        ...
                        <div class="c4"> 
                        <Icons>
                        <span class="view_icon material-symbols-outlined">visibility</span>
                        {item.number_views}</Icons>                       
                        <Icons>
                        <span class="heart_icon material-symbols-outlined">favorite</span>
                        {item.number_likes}</Icons>
                        <Icons><span class=" save_icon material-symbols-outlined">bookmark</span>{item.number_saves}</Icons>
</div>          
                    </div>
                    <div class="c5 c8">
                     ...
                     <div>
                    {
                        props.currentuser.id==item.author_id
                            ?
                                <>
                                    <Btn><Link to={`/edit/${item.id}`} style={{ textDecoration: 'none', color: 'black' }}>Edit</Link></Btn>
                                    <Btn onClick={()=>{deleteBlog(item.id)}}>Delete</Btn>
                                </>
